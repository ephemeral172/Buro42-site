...
export default function HiremiSection() {
  const { t } = useTranslation();
  const featureTexts = useMemo(() => t('hiremi.features', { returnObjects: true }), [t]);
  ...
  const techStack = useMemo(() => {
    const raw = t('hiremi.stack', { returnObjects: true });
      ...
      <div className="relative z-10 mx-auto max-w-6xl">
        ...
        >
          <div className="mb-4 flex items-center gap-4">
            <Briefcase className="h-6 w-6 text-ifi-lime" />
          ...
          <p className="max-w-3xl text-lg text-mineshaft-400">
            {t('hiremi.introBefore')}{' '}
            <a
              href="https://hiremi.ru"
              target="_blank"
        ...
        >
          <p className="inline-flex flex-wrap items-center justify-center gap-x-2 gap-y-1 rounded-full border border-ifi-border bg-mineshaft-900/70 px-5 py-3 font-mono text-sm text-mineshaft-400 backdrop-blur-sm">
            <Briefcase className="inline h-4 w-4 text-ifi-lime" />
            {t('hiremi.footer')}
          </p>
        </motion.div>
      </div>
