...
const featureMeta = [
  ...
  { icon: Monitor, color: 'text-ifi-lime' },
  { icon: Lock, color: 'text-sky-400' },
  { icon: Layers, color: 'text-violet-400' },
  { icon: FileSearch, color: 'text-amber-400' },
  { icon: Bell, color: 'text-rose-400' },
];
...
export default function ShadowGPTSection() {
  ...
  const featureTexts = useMemo(() => t('shadowgpt.features', { returnObjects: true }), [t]);
  ...
  const techStack = useMemo(() => {
    const raw = t('shadowgpt.stack', { returnObjects: true });
    ...
    <section className="relative px-4 py-20 md:py-24">
      <div className="relative z-10 mx-auto max-w-6xl">
        ...
        >
          ...
          <h2 className="mb-4 text-4xl font-bold tracking-tight text-ifi-fg lg:text-5xl">{t('shadowgpt.title')}</h2>
          <p className="max-w-3xl text-lg text-mineshaft-400">
            {t('shadowgpt.introBefore')}{' '}
            <a
