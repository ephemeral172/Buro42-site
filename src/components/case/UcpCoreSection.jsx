...
const featureMeta = [
  { icon: Shrink, color: 'text-cyan-400' },
  { icon: Hash, color: 'text-ifi-lime' },
  { icon: Server, color: 'text-sky-400' },
  { icon: RefreshCw, color: 'text-amber-400' },
  { icon: Braces, color: 'text-violet-400' },
  { icon: Boxes, color: 'text-rose-400' },
];
...
export default function UcpCoreSection() {
  const { t } = useTranslation();
  const featureTexts = useMemo(() => t('ucp.features', { returnObjects: true }), [t]);
  ...
  const techStack = useMemo(() => {
    const raw = t('ucp.stack', { returnObjects: true });
      ...
      <div className="relative z-10 mx-auto max-w-6xl">
        ...
        >
          <div className="mb-4 flex items-center gap-4">
            <PackageOpen className="h-6 w-6 text-ifi-lime" />
        ...
        >
          <p className="inline-flex flex-wrap items-center justify-center gap-x-2 gap-y-1 rounded-full border border-ifi-border bg-mineshaft-900/70 px-5 py-3 font-mono text-sm text-mineshaft-400 backdrop-blur-sm">
            <PackageOpen className="inline h-4 w-4 text-ifi-lime" />
            {t('ucp.footer')}
          </p>
        </motion.div>
      </div>
