...
export default function GlassCard({ children, className, innerClassName, compact }) {
  return (
    <div
      className={cn(
        'group/glass relative isolate h-full overflow-hidden rounded-[0.7rem]',
        className
      )}
    >
      {/* Мягкое пятно без «рамочного» квадрата — border-accent на отрицательном z-index давал артефакт при анимации opacity (FR/stacking). */}
      <span
        className={cn(
          'pointer-events-none absolute z-0 rounded-full bg-[#fab570]/30',
          'right-[7%] top-[28%]',
      ...
      <div
        className={cn(
          'relative z-[1] flex h-full min-h-0 flex-col overflow-hidden rounded-[0.7rem]',
